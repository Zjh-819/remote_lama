...
def run_thread(arguments):
    ...
    if arguments["interactive"]:
        ...
        # THIS IS OPTIONAL - mainly used for debugging reason
...
def main(args, shuffle_data=True, model=None):
    ...
    for i in tqdm(range(len(samples_batches))):
        ...
        (
            ...
            token_ids_list,
            masked_indices_list,
        ) = model.get_batch_generation(sentences_b, logger=logger)  # 这里就是生成填空

        if vocab_subset is not None:
            # filter log_probs 滤去交集里面没有的
        ...
        for idx, result in enumerate(res):
            ...
            if result_masked_topk['my_RANK'] > -1:
                precision_at_k[result_masked_topk['my_RANK']] += 1
    ...
    precision_at_k = (precision_at_k / len(list_of_results)).tolist()
    ...
    all_results = dict(
        list_of_results=list_of_results, global_MRR=MRR, global_P_at_10=Precision, global_P_at_k=precision_at_k
    )
